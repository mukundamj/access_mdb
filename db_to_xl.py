...
#For SE/SW/Weather station 

start =datetime.datetime(2015,03,28,00,00)
a=start
b=start+datetime.timedelta(0,240)

wb = load_workbook('..\SE_SW_Lysimeters_Weather_Mass_Experiment-02_03_26_2015.xlsx')
ws = wb.active
  
r1=r2=r3=7
while a < (start+datetime.timedelta(1)):
    params = (a,b)
    sql = "SELECT Date_Time, Address, Force, Flux1, Flux2 FROM SE_Data_5 WHERE Date_Time >= ? AND Date_Time <= ?"
    for row in cur.execute(sql,params):
        if row is None:
            continue
        if (row.get('Address') == 'A'):
            r = r1=r1+1
            ws.cell(row = r,column=1).value = row.get('Date_Time')
            ws.cell(row = r,column=7).value = row.get('Force')
            ws.cell(row = r,column=9).value = row.get('Flux1')
            ws.cell(row = r,column=10).value = row.get('Flux2')
#       print(u"Date_Time is {0} Address is {1} moisture is {2} temp is {3}".format(row.get('Date_Time'),row.get('Address'),row.get('Volumetric_Water_Content'), row.get('Soil_Temperature')))
    a = a+five_min
    b = b+five_min
...
while a < (start+datetime.timedelta(1)):
    params = (a,b)
    sql = "SELECT Date_Time, Address, Force, Flux1, Flux2 FROM SW_Data_5 WHERE Date_Time >= ? AND Date_Time <= ?"
    for row in cur.execute(sql,params):
        if row is None:
            continue
        if (row.get('Address') == 'A'):
            r = r1=r1+1
            ws.cell(row = r,column=2).value = row.get('Force')
            ws.cell(row = r,column=4).value = row.get('Flux1')
            ws.cell(row = r,column=5).value = row.get('Flux2')
    a = a+five_min
    b = b+five_min
...
r1=r2=r3=r4=r5=r6=r7=11
while a < (start+datetime.timedelta(1)):
    params = (a,b)
    sql = "SELECT Date_Time, CM3_Up, CG3_Up, CM3_Down, CG3_Down, NRM, Albedo, PT_100 FROM Lysimeter_Facility_Data_5 WHERE Date_Time >= ? AND Date_Time <= ?"
    for row in cur.execute(sql,params):
        if row is None:
            continue
        r = r1=r1+1
        ...
        ws.cell(row = r,column=16).value = row.get('NRM')
        ws.cell(row = r,column=17).value = row.get('Albedo')
        ws.cell(row = r,column=18).value = row.get('PT_100')
    a = a+five_min
    b = b+five_min
